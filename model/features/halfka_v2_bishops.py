...
def halfka_bishop_idx(is_white_pov: bool, king_sq: int, sq: int, p: chess.Piece) -> int:
    perspective = 1 - is_white_pov
    flip = 56 * perspective
    orient_s: int = sq ^ OrientTBL[king_sq] ^ flip

    offset: int = PIECE_SQUARE_INDEX[is_white_pov][p.color][p.piece_type]
    offset += ((p.piece_type == chess.BISHOP) & IS_BLACK_SQUARE[sq]) << 6;

    return orient_s + offset + KING_BUCKETS[king_sq ^ flip]


def halfka_bishop_psqts() -> list[int]:
    piece_values = {
        chess.PAWN:    208,
        chess.KNIGHT:  781,
        chess.BISHOP:  825,
    ...
    for ksq in range(64):
        for s in range(64):
            for pt, val in piece_values.items():
                idxw = halfka_bishop_idx(True, ksq, s, chess.Piece(pt, chess.WHITE))
                idxb = halfka_bishop_idx(True, ksq, s, chess.Piece(pt, chess.BLACK))
                values[idxw] = val
                values[idxb] = -val
...
class FactorizedFeatures(FeatureBlock):
    ...
    def get_initial_psqt_features(self) -> list[int]:
      return halfka_bishop_psqts() + [0] * PS_VIRTUAL_NB
